...
function SpeakerEditor({ speaker, lookup, onClose, onSaved }: { speaker: Speaker | null; lookup: Lookup; onClose: () => void; onSaved: (savedSpeaker?: Speaker) => void }) {
  const [form, setForm] = useState({ full_name: speaker?.full_name ?? "", credentials: speaker?.credentials ?? "", email: speaker?.email ?? "", title: speaker?.title ?? "", contact_info: speaker?.contact_info ?? "", bio: speaker?.bio ?? "", is_active: speaker?.is_active ?? true, seminar_title: "", seminar_description: "", category_id: "", department: "", status_id: "" })
  ...
  return (
    <div className="fixed inset-0 z-[70] flex justify-end bg-[var(--navy-950)]/35" role="dialog" aria-modal="true" aria-labelledby="speaker-editor-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <div className="h-full w-full max-w-2xl overflow-y-auto bg-white p-5 shadow-[var(--shadow-lg)] sm:p-8">
        <div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-4">{speaker && <SpeakerPhoto name={speaker.full_name} url={speaker.headshot_url} className="h-20 w-20 text-xl" />}<div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--green-600)]">Speaker management</p><h2 id="speaker-editor-title" className="mt-2 text-2xl font-bold [overflow-wrap:anywhere]">{speaker ? `Edit ${speaker.full_name}` : "Add a speaker"}</h2></div></div><button type="button" onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-[var(--radius-input)] border border-[var(--border)]" aria-label="Close editor"><X className="h-5 w-5" /></button></div>
  ...
  return (
    <div><SectionHeader eyebrow="Directory management" title="Speakers" description="Select any speaker card or its pencil icon to edit details, photos, availability, and seminars." actions={<><ExcelTools speakers={filtered} lookups={lookup} onImported={() => void load()} /><button type="button" onClick={() => setEditing(null)} className="inline-flex h-10 items-center gap-2 rounded-[var(--radius-button)] bg-[var(--green-600)] px-4 text-sm font-bold text-white"><Plus className="h-4 w-4" /> Add speaker</button></>} />
      ...
      {loading ? (
        ...
        <div className="mt-6 grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{filtered.map((speaker) => <SpeakerCard key={speaker.speaker_id} speaker={speaker} onEdit={() => setEditing(speaker)} />)}</div>
      ) : (
