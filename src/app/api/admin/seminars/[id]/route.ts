...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await admin.from("seminars").update({ title, description: cleanText(body.description, 5000) || null, category_id: optionalInteger(body.category_id), department_id: optionalInteger(body.department_id), status_id: optionalInteger(body.status_id), scheduled_at: cleanText(body.scheduled_at, 40) || null }).eq("seminar_id", id).select("*").single()
