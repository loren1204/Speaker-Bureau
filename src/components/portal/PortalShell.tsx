...
import { Activity, BookOpen, LayoutDashboard, Menu, Mic2, X, Inbox, Bell, LogOut } from "lucide-react"
...
export function PortalShell({ profile, children }: { profile: TeamProfile; children: React.ReactNode }) {
  ...
  const navigation = (
    <>
      <Link href="/speakers" className="flex h-20 items-center border-b border-[var(--border)] px-6">
      ...
      <div className="border-t border-[var(--border)] p-4">
        ...
        <button type="button" onClick={signOut} className="mt-2 flex min-h-10 w-full items-center gap-3 rounded-[var(--radius-input)] px-3 text-sm font-semibold text-[var(--text-muted)] hover:bg-[var(--canvas-subtle)] hover:text-[var(--navy-950)]">
    ...
    <div className="min-h-screen bg-[var(--canvas-subtle)] text-[var(--navy-950)] lg:grid lg:grid-cols-[260px_minmax(0,1fr)]">
      ...
      <div className="min-w-0 lg:col-start-2">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[var(--border)] bg-white/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          ...
          <p className="hidden text-sm font-semibold text-[var(--text-muted)] sm:block">Lee Health Speakers Bureau</p>
